# React Component Challenge — `foia-response-pipeline` (Cohort #1 Pair 3)

> **Phase 2 React-competency verification.** FOIA request processing aspect
> (5 USC 552; 28 CFR 16; anchor system **FOIA.gov / DOJ OIP**). Primary entity
> `FoiaRequest`, review entity `RedactionReview`. Multi-agent shape: redaction
> proposer + reviewer, **HITL on irreversible release**.
>
...
client (here: a FOIA officer making an irreversible release/withhold call). If a
generator wrote your `useState` and you can't say why the state lives there, you
will not survive that room. The hand-written constraint is the only way to
measure the thing we actually care about. It is loud, it is mandatory, and it is
attested.
...
3. **Real user interaction on the frontend.** A user does something — submits,
   triages, redacts, approves a release, flags an exemption — and sees the result.
...
| `foia-request-service` | `:8081` | `/api/foia-requests` | `FoiaRequest` lifecycle (intake → triage → response) |
| `redaction-review-service` | `:8082` | `/api/redaction-reviews` | `RedactionReview` coordination |
| `ai-orchestrator` (FastAPI) | `:8000` | `/api/ai/*` | LLM / RAG / multi-agent redaction proposer + reviewer |
...
the UI against it. **Critical domain rule:** release/withhold is *irreversible*
and HITL-gated (see `docs/hitl-plan.md`) — a component that triggers a release
must require explicit human confirmation, never auto-release.
...
[`docs/adrs/0004-react-component-challenge.md`](./adrs/0004-react-component-challenge.md).

- **Number: `0004`.** This repo's `docs/adrs/` holds the aspect-commitment +
  microservices `0001`s, `0002-bedrock-as-llm-anchor`, and
  `0003-retrieval-baseline`. `0004` is the next sequential ADR. Keep the
  `# ADR 0004 — Title` heading, `Status:` / `Date:` header lines, and the
...
points for the FOIA aspect — but you may **propose your own** if it meets the
requirements and you defend the choice in the ADR.

1. **Redaction-Review Queue.** A queue of `RedactionReview` items where the
   `ai-orchestrator` redaction-proposer has suggested exemption codes (B1–B9)
   on document spans; the reviewer accepts, overrides, or escalates each
   proposal, and an irreversible **release** is gated behind explicit
   confirmation (per the HITL plan). Server state (the queue + proposals) +
   client state (the in-flight per-span decision) + a derived "ready to release"
   verdict.
2. **FOIA Request Intake/Triage Panel.** A filterable list of incoming
   `FoiaRequest`s with AI-surfaced triage hints (track assignment, due-date
   computation under the 20-business-day clock); the officer assigns a track and
   sets status. Server state (the request list) + client state (filters, the
   triage form draft).
3. **Exemption-Citation Card.** For one document span, call the RAG path over
   the FOIA authority corpus (5 USC 552 / 28 CFR 16) and render candidate
   exemptions with citations and confidence, letting the reviewer confirm or
   reject each. Exercises an async AI call, loading/streaming states, a derived
   recommendation, and a clear empty/error state.
...
- **Derived vs server state** — what is computed on every render from existing
  state/props (derived, not stored — e.g. the "ready to release" verdict) vs
  what comes from the backend (server state — the queue, the proposals). This
...
- [ ] **Failure behavior — resilience over happy-path.** How does the component
      ...
      - the response is **slow** (does the UI block, show a skeleton, allow
        cancel)?
      - the result is **empty** (no requests queued, no redaction proposals)?
      - the backend returns an **error** (4xx/5xx, malformed payload)?
- [ ] **Irreversibility / HITL.** If your component can trigger a release, how do
      you guarantee no accidental auto-release? Show the human-confirmation gate.
...
graded above features — and here, the irreversible-release gate is non-optional.
...
- [ ] If it can release, the irreversible action is behind an explicit HITL gate.
- [ ] ADR `0004` is filled in (Context / Decision / Consequences / Alternatives),
...
- **Codex review** — independent diff review for correctness and resilience
  (error/empty/loading paths actually present; release gate present if
  applicable), and a smell-check for unexplained generated-looking scaffolding.
  Never silently skipped; if Codex is unavailable the review runs in
  `degraded-review` mode with explicit acknowledgment.
