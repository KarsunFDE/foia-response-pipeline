...
## Item 2 — Audit-log race in foia-request-service

- **Where:** `services/foia-request-service/.../service/FoiaRequestService.java` + `.../audit/AuditLogger.java`
...
## Item 3 — No circuit breaker in redaction-review-service → foia-request-service

- **Where:** `services/redaction-review-service/.../client/FoiaRequestClient.java`
- **How found:** Load test the redaction_review endpoint while foia-request-service is slow; threads pile up. No `@CircuitBreaker`, no `@TimeLimiter`, no Resilience4j config in `application.yml`.
...
- **Where:** `services/ai-orchestrator/app/main.py` `/draft-foia-request` endpoint
...
- **Where:**
  ...
  - `foia-request-service`: logs `correlationId`
  - `redaction-review-service`: logs `traceId`
...
- **Where:** `frontend/src/app/components/foia_request-list/foia_request-list.component.ts`
...
## Item 9 — No OWASP input sanitization on foia_request `description`

- **Where:** `services/foia-request-service/.../dto/FoiaRequestCreateRequest.java` + `.../service/FoiaRequestService.java`
- **How found:** POST a foia_request with `<script>alert(1)</script>` in the `description` field; it's stored verbatim and returned on GET. No `Jsoup.clean()`, no allow-list, no `@SafeHtml`.
- **Surfaces in:** W4 Wed AI Security Engineering Day (prompt-injection-via-stored-content — foia_request descriptions feed the ai-orchestrator prompt).
- **Fixed looks like:** Jsoup allow-list sanitization on write; output-encoding on read.

## Item 10 — No multi-tenant boundary

- **Where:** `services/foia-request-service/.../model/FoiaRequest.java` + `.../repository/FoiaRequestRepository.java`
- **How found:** Schema has `agency_id` field, but `FoiaRequestRepository.findAll()` returns *all* foia_requests across agencies; no `findByAgencyId` in use.
