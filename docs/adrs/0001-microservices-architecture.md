...
- **FoiaRequest Service** (Spring Boot) for FAR/DFARS foia_request lifecycle.
- **RedactionReview Service** (Spring Boot) for redaction_review-panel coordination.
...
- **Known weakness:** the boundary between foia-request-service and redaction-review-service is *deliberately under-defined* — the cohort discovers it as a refactor target.
