...
Item 4 (docs/brownfield-debt.md#item-4): /draft-foia-request returns the raw
...
After fix:
  ...
  - /draft-foia-request declares response_model=DraftResponse (strict mode)
...
def test_draft_foia_request_has_pydantic_response_model_DEBT_LOCKED() -> None:
    """When item 4 is modernized, /draft-foia-request declares a Pydantic BaseModel response_model.
    ...
    for route in app.routes:
        if getattr(route, "path", None) == "/draft-foia-request":
            found_route = route
            break
    assert found_route is not None, "/draft-foia-request route not registered"
    ...
    assert is_pydantic_model, (
        f"Item 4 modernized — /draft-foia-request must declare a Pydantic "
...
@pytest.mark.brownfield_debt_4
def test_draft_foia_request_never_returns_null_clause_id_DEBT_LOCKED() -> None:
