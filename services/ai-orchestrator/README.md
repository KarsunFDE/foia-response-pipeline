...
| POST   | `/draft-foia-request` | Bedrock stub; ⚠ Item 4 — returns raw JSON, sometimes `{"clause_id": null}` |
| POST   | `/draft-foia-request-v1` | v1.0 composed-Runnable scaffold (Item 5 modernization target) |
...
- **Item 4** — `/draft-foia-request` returns raw stub JSON; no Pydantic
...
- **Item 7** — `pinecone-client` in `requirements.txt`, no `import pinecone`
  anywhere.
- **Item 11 (partial)** — `Dockerfile` was originally `FROM python:latest`; pinned to `python:3.11-slim` in 2026-Q1 after numpy/pydantic-core wheels broke on 3.14. The OTHER 4 Dockerfiles (api-gateway, foia-request-service, redaction-review-service, frontend) still carry `:latest`. Cohort finds those.
