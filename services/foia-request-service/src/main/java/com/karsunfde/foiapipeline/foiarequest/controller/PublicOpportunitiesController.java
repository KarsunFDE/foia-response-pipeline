...
import com.karsunfde.foiapipeline.foia_request.service.AmendmentService;
...
public class PublicOpportunitiesController {
    ...
    @Autowired
    public PublicOpportunitiesController(FoiaRequestService svc,
    ...
    @GetMapping
    public List<FoiaRequest> list(@RequestParam(required = false) String naics,
    ...
    public ResponseEntity<Map<String, Object>> detail(@PathVariable String id) {
        return svc.findById(id).map(s -> {
            ...
            body.put("foia_request", s);
            List<Amendment> amendments = amendmentSvc.listForFoiaRequest(id);
