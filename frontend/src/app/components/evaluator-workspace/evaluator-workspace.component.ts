...
import { RedactionReviewScore } from '../../models/redaction_review';

/**
 * Evaluator Workspace (TEP member view).
 *
 * Assigned proposals only (Section M factor-by-factor; narrative
 * required). Mirrors FAR 15.305 redaction_review discipline + Item 3
 * surface (evaluator-agent calling foia-request-service for proposal
  ...
  template: `
    <div class="page-header">
      <div>
        ...
        <div class="subtitle">{{ role.current.displayName }} · Eval {{ redaction_review.id }} · FAR 15.305</div>
      </div>
      <a [routerLink]="['/redaction_review', redaction_review.id, 'consensus']"><button class="secondary">Consensus view →</button></a>
    </div>
    ...
    <div class="card">
      ...
      <table>
        <thead>
          <tr>
            <th>Proposal</th>
            <th *ngFor="let f of redaction_review.factors">
        ...
        <tbody>
          <tr *ngFor="let p of proposals">
            ...
            <td *ngFor="let f of redaction_review.factors">
...
export class EvaluatorWorkspaceComponent {
  ...
  scoreCache: Record<string, RedactionReviewScore> = {};
  ...
  scoreFor(proposalId: string, factorId: string): RedactionReviewScore {
