 ...
 * against foia-response-pipeline (incl. Item 12 — repo's own lint debt).
 *
      ...
      <div class="two-col" style="grid-template-columns:1fr 1fr 1fr">
        <label><span class="label-text">Scope</span>
          <select [(ngModel)]="draft.scope">
            ...
            <option value="PLATFORM">Platform (foia-response-pipeline)</option>
          </select>
        </label>
...
export class FindingsTrackerComponent implements OnInit {
  ...
  openFinding(): void {
    const f: Finding = {
      ...
      scopeId: this.draft.scopeId ?? 'foia-response-pipeline',
